...
// Forecast removed - integrated into Feedback
...
export const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  const tabs = [
    { id: 'trading' as const, label: t.trading, icon: '📈' },
    { id: 'signals' as const, label: t.signals, icon: '🎯' },
    { id: 'journal' as const, label: t.journal, icon: '📝' },
    { id: 'feedback' as const, label: t.feedback, icon: '🧠' },
    { id: 'analysis' as const, label: t.analysis, icon: '📊' },
    { id: 'backtest' as const, label: t.backtest, icon: '🔬' },
    { id: 'news-settings' as const, label: t.news, icon: '📰' },
    { id: 'settings' as const, label: t.settings, icon: '⚙️' },
    { id: 'user-settings' as const, label: t.profile, icon: '👤' },
  ];

  return (
    <div className={`h-screen flex flex-col bg-[#131722] ${isRTL ? 'rtl' : 'ltr'}`}>
      {/* Ticker Tape */}
            ...
            {activeTab === 'trading' && (
              <div className="h-full flex flex-col">
                ...
                <div className="flex-shrink-0 border-t border-[#2a2a3e]">
                  ...
                  >
                    <span className="text-sm font-medium text-white">💼 {t.portfolio}</span>
                    <span className="text-gray-400 text-xs">
                      {showPortfolio ? `▼ ${t.hide}` : `▶ ${t.show}`}
                    </span>
                  </button>
                  {showPortfolio && (
                    <div className="h-64">
