...
export const LoginForm: React.FC<LoginFormProps> = ({ onLogin }) => {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 ${language === 'fa' ? 'rtl' : 'ltr'}`}>
      ...
      <div className="relative max-w-md w-full mx-4">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white">{t('messages.welcome')}</h1>
          <p className="text-purple-200 mt-2">{language === 'fa' ? 'سیگنال‌های هوشمند ترید' : 'Intelligent Trading Signals'}</p>
        </div>

        {/* Login Card */}
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-white/20">
          <h2 className="text-xl font-semibold text-white mb-6">{t('nav.login')}</h2>
          ...
          <div className="mt-6 text-center">
            <p className="text-purple-200/60 text-sm">
              {language === 'fa' ? 'نام کاربری: myazdanpanah / رمز: 123456' : 'Demo credentials: myazdanpanah / 123456'}
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-3 gap-4">
          {[
            { icon: '📊', label: language === 'fa' ? 'سیگنال زنده' : 'Real-time Signals' },
            { icon: '🤖', label: language === 'fa' ? 'تحلیل هوش مصنوعی' : 'AI Analysis' },
            { icon: '📈', label: language === 'fa' ? 'عملکرد' : 'Performance' },
          ].map((feature, i) => (
