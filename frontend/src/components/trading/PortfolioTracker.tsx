    ...
    <div className="bg-[#131722] h-full flex flex-col rounded-lg border border-gray-700">
      ...
      {stats && (
        <div className="px-4 py-3 border-b border-gray-700 flex items-center gap-6">
          ...
          <div>
            ...
            <div className={`text-lg font-mono ${stats.totalPnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {stats.totalPnl >= 0 ? '+' : ''}{stats.totalPnlPercent.toFixed(2)}%
            </div>
          </div>
          <div>
            <div className="text-[10px] text-gray-500">Unrealized</div>
            <div className={`text-sm font-mono ${stats.unrealizedPnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden">
        <div className="flex h-full">
          {positions.map((pos, i) => (
            <div key={pos.symbol} className="flex-shrink-0 w-48 px-4 py-3 border-r border-gray-700 hover:bg-gray-800/50 flex flex-col justify-center">
